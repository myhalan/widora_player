...
void LCD_ShowString(uint16_t x,uint16_t y,const char *p,uint8_t mode);		 //显示一个字符串,16字体
...
void display_hzk16(unsigned int x,unsigned int y,char* const chs,uint8_t mode);
...
void drawcircle_field(uint16_t x0,uint16_t y0,uint8_t out_r,uint8_t in_r);

void LCD_SetCursor(uint16_t Xpos, uint16_t Ypos);


//画笔颜色(这些颜色是RGB的，要转换为BGR才能正确显示)
